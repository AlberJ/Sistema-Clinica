<?php /* Smarty version Smarty-3.1.18, created on 2017-11-23 03:57:15
         compiled from "C:\wamp\www\sistemaclinica\templates\ConsultaListView.tpl" */ ?>
<?php /*%%SmartyHeaderCode:309045a16471be52a41-71109069%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '22ebbd44b43a9873dd194f9202bf3994a1edf011' => 
    array (
      0 => 'C:\\wamp\\www\\sistemaclinica\\templates\\ConsultaListView.tpl',
      1 => 1511420168,
      2 => 'file',
    ),
  ...
  'nocache_hash' => '309045a16471be52a41-71109069',
  'function' => 
  array (
  ),
  'variables' => 
  ...
  'unifunc' => 'content_5a16471c00cce2_64310688',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5a16471c00cce2_64310688')) {function content_5a16471c00cce2_64310688($_smarty_tpl) {?><?php if (!is_callable('smarty_modifier_date_format')) include 'C:\\wamp\\www\\phreeze\\libs\\smarty\\plugins\\modifier.date_format.php';
?><!DOCTYPE html>
<html lang="en">
	<head>
...
" />
		<title>BANCOCLINICAMODELO | Consultas</title>
...
<script type="text/javascript">
	...
	<?php $_smarty_tpl->tpl_vars["nav"] = new Smarty_variable("consultas", null, 0);?>
	...
	<script type="text/template" id="consultaCollectionTemplate">
		...
		<thead>
			<tr>
				...
				<th id="header_Dataconsulta">Dataconsulta<<?php ?>% if (page.orderBy == 'Dataconsulta') { %<?php ?>> <i class='icon-arrow-<<?php ?>%= page.orderDesc ? 'up' : 'down' %<?php ?>>' /><<?php ?>% } %<?php ?>></th>
				<th id="header_Codpaciente">Codpaciente<<?php ?>% if (page.orderBy == 'Codpaciente') { %<?php ?>> <i class='icon-arrow-<<?php ?>%= page.orderDesc ? 'up' : 'down' %<?php ?>>' /><<?php ?>% } %<?php ?>></th>
				<th id="header_Crmmedico">Crmmedico<<?php ?>% if (page.orderBy == 'Crmmedico') { %<?php ?>> <i class='icon-arrow-<<?php ?>%= page.orderDesc ? 'up' : 'down' %<?php ?>>' /><<?php ?>% } %<?php ?>></th>
				<th id="header_Descricaoconsulta">Descricaoconsulta<<?php ?>% if (page.orderBy == 'Descricaoconsulta') { %<?php ?>> <i class='icon-arrow-<<?php ?>%= page.orderDesc ? 'up' : 'down' %<?php ?>>' /><<?php ?>% } %<?php ?>></th>
			</tr>
		</thead>
		<tbody>
		<<?php ?>% items.each(function(item) { %<?php ?>>
			<tr id="<<?php ?>%= _.escape(item.get('codconsulta')) %<?php ?>>">
				<td><<?php ?>%= _.escape(item.get('codconsulta') || '') %<?php ?>></td>
				<td><<?php ?>%if (item.get('dataconsulta')) { %<?php ?>><<?php ?>%= _date(app.parseDate(item.get('dataconsulta'))).format('MMM D, YYYY') %<?php ?>><<?php ?>% } else { %<?php ?>>NULL<<?php ?>% } %<?php ?>></td>
				<td><<?php ?>%= _.escape(item.get('codpaciente') || '') %<?php ?>></td>
				<td><<?php ?>%= _.escape(item.get('crmmedico') || '') %<?php ?>></td>
				<td><<?php ?>%= _.escape(item.get('descricaoconsulta') || '') %<?php ?>></td>
			</tr>
		<<?php ?>% }); %<?php ?>>
		</tbody>
		</table>
		...
		<form class="form-horizontal" onsubmit="return false;">
			<fieldset>
				...
				<div id="dataconsultaInputContainer" class="control-group">
					...
					<div class="controls inline-inputs">
						<div class="input-append date date-picker" data-date-format="yyyy-mm-dd">
							<input id="dataconsulta" type="text" value="<<?php ?>%= _date(app.parseDate(item.get('dataconsulta'))).format('YYYY-MM-DD') %<?php ?>>" />
				...
				<div id="descricaoconsultaInputContainer" class="control-group">
					...
					<div class="controls inline-inputs">
						<textarea class="input-xlarge" id="descricaoconsulta" rows="3"><<?php ?>%= _.escape(item.get('descricaoconsulta') || '') %<?php ?>></textarea>
						<span class="help-inline"></span>
					</div>
				</div>
			</fieldset>
